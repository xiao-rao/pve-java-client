package io.github.pve.client.resource.nodes.qemu.firewall.ipset;
...
import io.github.pve.client.model.nodes.qemu.firewall.ipset.*;

/**
 * Client for /nodes/{node}/qemu/{vmid}/firewall/ipset
 * BY '@xiao-rao'
 */
public class IpsetClient {
    ...
    public IpsetClient(ProxmoxApiExecutor executor, String node, String vmId) {
        ...
        this.node = node;
        this.vmId = vmId;
        this.basePath = "/nodes/{node}/qemu/{vmid}/firewall/ipset".replace("{" + "node" + "}", node).replace("{" + "vmid" + "}", vmId);
    }
