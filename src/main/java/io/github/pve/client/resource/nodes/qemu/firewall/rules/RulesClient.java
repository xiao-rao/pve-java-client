package io.github.pve.client.resource.nodes.qemu.firewall.rules;
...
import io.github.pve.client.model.nodes.qemu.firewall.rules.*;

/**
 * Client for /nodes/{node}/qemu/{vmid}/firewall/rules
 * BY '@xiao-rao'
 */
public class RulesClient {
    ...
    public RulesClient(ProxmoxApiExecutor executor, String node, String vmId) {
        this.executor = executor;
        this.node = node;
        this.vmId = vmId;
        this.basePath = "/nodes/{node}/qemu/{vmid}/firewall/rules".replace("{" + "node" + "}", node).replace("{" + "vmid" + "}", vmId);
    }
