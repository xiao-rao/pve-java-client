package io.github.pve.client.resource.nodes.lxc.status.reboot;
...
/**
 * Client for /nodes/{node}/lxc/{vmid}/status/reboot
 * BY '@xiao-rao'
 */
public class RebootClient {
    ...
    public RebootClient(ProxmoxApiExecutor executor, String node, String vmid) {
        ...
        this.node = node;
        this.vmid = vmid;
        this.basePath = "/nodes/{node}/lxc/{vmid}/status/reboot".replace("{" + "node" + "}", node).replace("{" + "vmid" + "}", vmid);
    }
